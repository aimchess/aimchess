...
export default function TournamentDetailsPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        const checkActiveGame = async () => {
            try {
                const res = await fetch(`/api/play/pending?tournamentId=${params.id}`);
                if (res.ok) {
                    const data = await res.json();
                    if (data.activeGameId) {
                        toast.success("Tournament Match Started! Redirecting to board...");
    ...
    const isPastStart = startDateObj <= new Date();

    return (
        <CRMShellLayout>
            <div className="space-y-6 max-w-5xl mx-auto">
                {/* Header Banner */}
                <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 rounded-2xl p-6 md:p-8 text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-xl relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 text-indigo-200 text-sm font-semibold">
                                ...
                                {startDateObj.toLocaleString(undefined, { weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 relative z-10 w-full md:w-auto">
                            {tournament.status === 'UPCOMING' && (
                                <button
                                    ...
                                    className="bg-emerald-500 hover:bg-emerald-400 text-stone-900 font-extrabold px-5 py-2.5 rounded-xl text-xs flex items-center gap-2 shadow-lg transition-all"
                                >
                                    {actionLoading ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />}
                                    Start & Auto-Pair Matches
                                </button>
                            )}
                            {tournament.status === 'ONGOING' && (
                                <>
                                    <button
                                        ...
                                        disabled={actionLoading}
                                        className="bg-amber-500 hover:bg-amber-400 text-stone-900 font-extrabold px-4 py-2.5 rounded-xl text-xs flex items-center gap-2 shadow-lg transition-all"
                                    >
                                        {actionLoading ? <Loader2 size={14} className="animate-spin" /> : <Swords size={14} />}
                                        Auto-Pair Active Matches
                                    </button>
                                    <button
                                        onClick={() => handleAdminAction("COMPLETE")}
                                        disabled={actionLoading}
                                        className="bg-rose-600 hover:bg-rose-500 text-white font-bold px-4 py-2.5 rounded-xl text-xs flex items-center gap-2 shadow-lg transition-all"
                                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-2 bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button
                                ...
                                className="text-xs text-gray-500 hover:text-indigo-600 flex items-center gap-1 font-bold"
                            >
                                        ...
                                        {tournament.participants.map((p: any, index: number) => {
                                            ...
                                            return (
                                                <tr key={p.id} className={`border-b hover:bg-gray-50/50 ${isMe ? 'bg-indigo-50/30' : ''}`}>
                                                    <td className="px-4 py-3 font-black text-gray-400">#{index + 1}</td>
                                                    ...
                                                    <td className="px-4 py-3 font-black text-base md:text-lg text-gray-900">{p.score}</td>
                                                    <td className="px-4 py-3 text-right">
                                                        {!isMe && isParticipant && tournament.status !== "COMPLETED" && (
                                                            ...
                                                            >
                                                                {challenging === p.userId ? <Loader2 size={14} className="animate-spin" /> : <Swords size={14} />}
                    ...
                    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-4 h-fit">
                        <h3 className="text-lg font-black text-gray-900 border-b pb-2">Tournament Info</h3>
