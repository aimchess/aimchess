...
export default function StudentDashboard() {
  ...
  const launchPuzzle = (puzzleId: string, context: 'TODO' | 'LIBRARY', nextPuzzleId?: string, dueDate?: string) => {
    ...
    const params = new URLSearchParams()
    if (nextPuzzleId) params.set('next', nextPuzzleId)
    if (context === 'TODO') params.set('context', 'todo')
    router.push(`/puzzle/${puzzleId}?${params.toString()}`)
  }

  // --- NEW: FUNCTION TO OPEN COURSES ---
  const launchCourse = (courseId: string) => {
    // Assuming you have a classroom or course viewing route
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-10 pt-20 md:pt-24 px-4 md:px-8">
      <div className="max-w-7xl mx-auto mb-8">
        
        {/* HEADER SECTION */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-4xl font-black text-slate-800 tracking-tighter uppercase">Student Classroom</h1>
            <p className="text-slate-500 mt-1 font-bold text-sm uppercase tracking-widest">Instructor: Royal Rook Academy</p>
          </div>
          <button 
            onClick={fetchAssignments} 
            className="w-full md:w-auto flex items-center justify-center gap-2 bg-white text-slate-600 font-bold px-6 py-3 rounded-2xl shadow-sm border hover:bg-slate-50 transition active:scale-95"
          >
            <RefreshCcw size={18} className={isRefreshing ? "animate-spin" : ""} />
            <span className="md:hidden">Refresh Content</span>
          </button>
        </div>

        {/* NAVIGATION TABS */}
        <div className="mt-8 flex gap-2 border-b border-slate-200 overflow-x-auto no-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
          {[
            { id: 'TODO', label: 'To Do', icon: Clock, color: 'orange', count: pending.length },
            { id: 'HISTORY', label: 'History', icon: History, color: 'slate' },
            { id: 'LIBRARY', label: 'Library', icon: BookOpen, color: 'blue' },
            { id: 'FEES', label: 'Fees', icon: CreditCard, color: 'green' },
            { id: 'SCHEDULE', label: 'Schedule', icon: Calendar, color: 'purple' },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as any)}
              className={`flex items-center gap-2 pb-4 border-b-2 text-[11px] md:text-xs font-black uppercase tracking-widest px-4 whitespace-nowrap transition-all ${
                activeTab === tab.id 
            ...
            >
              <tab.icon size={16} />
              {tab.label}
              {tab.count ? <span className="bg-orange-100 text-orange-600 text-[10px] px-2 py-0.5 rounded-full">{tab.count}</span> : null}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* TODO TAB */}
        {activeTab === 'TODO' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 animate-in fade-in slide-in-from-bottom-4">
            {pending.length === 0 && (
              <div className="col-span-full py-20 bg-white rounded-[2rem] border-2 border-dashed border-slate-200 text-center px-6">
                <CheckCircle className="w-16 h-16 text-green-200 mx-auto mb-4" />
                <h3 className="text-xl font-black text-slate-800 uppercase">Training Complete!</h3>
                <p className="text-slate-500 font-bold text-sm mt-2 uppercase tracking-wide">No pending assignments for today.</p>
              </div>
            )}
            {pending.map((item, index) => {
              const nextId = pending[index + 1]?.puzzle.id;
              const isOverdue = item.dueDate && new Date() > new Date(item.dueDate);
              return (
                <div
                  key={item.id}
                  onClick={() => launchPuzzle(item.puzzle.id, 'TODO', nextId, item.dueDate)}
                  className="group bg-white rounded-[2rem] p-6 border shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer relative overflow-hidden active:scale-95"
                >
                  <div className={`absolute top-0 left-0 w-full h-1.5 ${isOverdue ? 'bg-red-500' : 'bg-orange-500'}`} />
                  <div className="flex justify-between items-start mb-6">
                    <div className={`${isOverdue ? 'bg-red-50 text-red-600' : 'bg-orange-50 text-orange-600'} p-4 rounded-2xl transition-colors group-hover:bg-orange-600 group-hover:text-white`}>
                      <PlayCircle size={28} />
                    </div>
                    <div className="text-right">
                      <span className={`text-[10px] font-black px-2 py-1 rounded-lg uppercase tracking-widest ${isOverdue ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500'}`}>
                        {isOverdue ? 'Expired' : 'Active'}
                      </span>
                    </div>
                  </div>
                  <h3 className="text-xl font-black text-slate-800 uppercase leading-tight group-hover:text-orange-600 transition-colors">{item.puzzle.title}</h3>
                  <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-50">
                    <Clock size={14} className="text-slate-400" />
                    <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider">
                      Due: {item.dueDate ? new Date(item.dueDate).toLocaleDateString() : 'No Limit'}
                    </span>
                  </div>
                </div>
              )
        ...
        {/* LIBRARY TAB */}
        {activeTab === 'LIBRARY' && (
          <div className="bg-white rounded-[2.5rem] p-4 md:p-8 border shadow-sm animate-in fade-in">
              ...
              {STAGE_ORDER.map((stage) => {
                ...
                return (
                  <button 
                    ...
                    disabled={locked} 
                    className={`px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest border flex items-center gap-2 transition-all whitespace-nowrap ${
                      libraryStage === stage ? 'bg-slate-900 text-white border-slate-900' : locked ? 'bg-slate-50 text-slate-300 cursor-not-allowed grayscale' : 'bg-white text-slate-500 hover:border-blue-500'
                    }`}
                  >
                    {stage} {locked && <Lock size={12} />}
                  </button>
            ...
            {/* BREADCRUMBS */}
            <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-8 overflow-x-auto no-scrollbar whitespace-nowrap">
              <span onClick={() => handleBreadcrumbClick(-1)} className="cursor-pointer hover:text-blue-600">Library</span>
              {curriculumPath.map((f, i) => (
                <div key={f.id} className="flex items-center gap-2">
                  <ChevronRight size={14} />
                  <span onClick={() => handleBreadcrumbClick(i)} className="cursor-pointer hover:text-blue-600">{f.name}</span>
                </div>
              ))}
            </div>

            {/* COURSES LISTING - FIXED OPEN LOGIC */}
            {curriculumPath.length === 0 && courses.filter(c => c.level === libraryStage).length > 0 && (
              <div className="mb-10 animate-in fade-in">
                <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                  <Star className="text-orange-500 fill-orange-500" size={16} /> Specialized Courses
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {courses.filter(c => c.level === libraryStage).map(course => (
                    <div 
                      ...
                      className="group bg-slate-50 rounded-[2rem] p-6 border-2 border-transparent hover:border-blue-500 hover:bg-white hover:shadow-2xl transition-all cursor-pointer flex flex-col h-full active:scale-95"
                    >
                      <div className="flex justify-between items-start mb-6">
                        <div className="p-4 bg-white rounded-2xl shadow-sm group-hover:bg-blue-600 group-hover:text-white transition-colors">
                          <BookOpen size={24} />
                        </div>
                        {course.audioUrl && <Volume2 size={20} className="text-blue-500" />}
                      </div>
                      <h4 className="text-lg font-black text-slate-800 uppercase leading-tight mb-2">{course.title}</h4>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wide line-clamp-2 mb-6">{course.description}</p>
                      <div className="mt-auto pt-6 border-t border-slate-100 flex justify-between items-center">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{course.chapters?.length || 0} Modules</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {curriculumItems.folders.map(f => (
                <div key={f.id} onClick={() => handleFolderClick(f)} className="aspect-square bg-white border-2 border-slate-50 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:shadow-xl hover:border-blue-500 transition-all active:scale-95 group">
                  <Folder className="w-10 h-10 text-blue-400 mb-3 group-hover:scale-110 transition-transform" />
                  <span className="text-[10px] font-black text-slate-800 uppercase tracking-tight text-center px-4">{f.name}</span>
                </div>
              ))}
              {curriculumItems.puzzles.map((p, idx) => {
                const isSolved = puzzleProgress[p.id]?.isSolved;
                return (
                  <div
                    key={p.id}
                    onClick={() => launchPuzzle(p.id, 'LIBRARY', curriculumItems.puzzles[idx+1]?.id)}
                    className={`p-4 border-2 rounded-[1.5rem] cursor-pointer transition-all active:scale-95 flex flex-col justify-between aspect-square ${
                      isSolved ? 'bg-green-50 border-green-200' : 'bg-white border-slate-50 hover:border-orange-500 shadow-sm'
                    }`}
                  >
                    {isSolved ? <CheckCircle className="text-green-600" /> : <FileText className="text-slate-300" />}
                    <span className={`text-[10px] font-black uppercase tracking-tight leading-snug ${isSolved ? 'text-green-800' : 'text-slate-800'}`}>
                      {p.title}
                    </span>
                  </div>
                )
            ...
            {curriculumItems.puzzles.length === 0 && curriculumItems.folders.length === 0 && (
               <div className="py-20 text-center text-slate-300 font-bold uppercase tracking-widest text-xs border-2 border-dashed rounded-[2rem]">
                 Empty Segment
               </div>
            )}
          </div>
        ...
        {activeTab === 'SCHEDULE' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 animate-in fade-in">
            {classes.map(c => (
              <div key={c.id} className="bg-white rounded-[2rem] p-6 border shadow-sm hover:shadow-xl transition-all">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-xl font-black text-slate-800 uppercase">{c.name}</h3>
                    <div className="flex items-center gap-2 text-purple-600 font-black text-[10px] uppercase tracking-widest mt-1">
                      <Calendar size={14} /> {c.dayOfWeek}
                    </div>
                  </div>
                  <div className="p-3 bg-purple-50 text-purple-600 rounded-2xl"><ClockIcon size={24} /></div>
                </div>
                <div className="space-y-3 mb-8">
                  <div className="flex items-center gap-3 text-sm font-bold text-slate-600">
                    <span className="w-20 uppercase tracking-widest text-[10px] text-slate-400">Time:</span>
                    {c.startTime} - {c.endTime}
                  </div>
                  <div className="flex items-center gap-3 text-sm font-bold text-slate-600">
                    <span className="w-20 uppercase tracking-widest text-[10px] text-slate-400">Coach:</span>
                    {c.coach?.name || 'N/A'}
                  </div>
                </div>
                {c.meetingLink ? (
                  <a href={c.meetingLink} target="_blank" className="w-full flex items-center justify-center gap-2 bg-slate-900 text-white font-black uppercase text-xs tracking-widest py-4 rounded-2xl hover:bg-purple-600 transition-all active:scale-95">
                    <Video size={18} /> Join Video Class
                  </a>
                ) : (
                  <div className="w-full bg-slate-100 text-slate-400 font-black uppercase text-xs tracking-widest py-4 rounded-2xl text-center">Offline/No Link</div>
                )}
              </div>
            ))}
          </div>
        ...
        {activeTab === 'FEES' && (
          <div className="bg-white rounded-[2.5rem] border shadow-sm overflow-hidden animate-in fade-in">
             <div className="p-8 border-b bg-slate-50 flex items-center gap-4">
               <div className="p-4 bg-green-100 text-green-600 rounded-2xl shadow-sm"><Wallet size={32} /></div>
               <div>
                  <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">Ledger Records</h2>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Academic Year 2024-25</p>
               </div>
             </div>
             <div className="overflow-x-auto">
               <table className="w-full text-left border-collapse min-w-[600px]">
                 <thead>
                   <tr className="bg-slate-50 border-b">
                     ...
                     <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-widest">Method</th>
                 ...
                 <tbody className="divide-y divide-slate-100">
                   {payments.map(p => (
                     <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                       <td className="p-6 text-sm font-bold text-slate-700">{new Date(p.date).toLocaleDateString()}</td>
                       <td className="p-6 font-black text-lg text-slate-800">₹{p.amount.toLocaleString()}</td>
                       <td className="p-6 font-bold text-[10px] uppercase text-slate-400">{p.method}</td>
               ...
               {payments.length === 0 && <div className="py-20 text-center font-bold text-slate-300 uppercase italic">No records found</div>}
             </div>
          </div>
        )}
