            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all ${
                  selectedCategory === cat ? 'bg-[#E76F51] text-white' : 'bg-white text-[#2D2A26] border border-[#E6E0D4]'
                }`}
              >
                {cat}
              </button>
            ...
            <div className="space-y-2">
              {categories.map(cat => (
                <button
                  key={cat}
                  onClick={() => setSelectedCategory(cat)}
                  className={`w-full text-left px-4 py-3 rounded-xl font-bold text-sm flex justify-between items-center transition-all ${
                    selectedCategory === cat ? 'bg-[#2D2A26] text-white' : 'hover:bg-[#FDFBF7] text-[#5C5852]'
                  }`}
                >
                  <span>{cat}</span>
